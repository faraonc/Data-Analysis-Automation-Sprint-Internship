...
def main():
	try:
		...
		csvfile = open(args.csv_stream)
		csv_reader = csv.reader(csvfile)
		streams, times, ip_src, ip_dst = getData(csv_reader)
		...
		print("process_stream_list.py Elapsed Time: ", (end - start), " s\n")
	
	finally:
		csvfile.close()

def getData(csv_reader):
	tmp_streams = []
	tmp_times = []
	tmp_src = []
	tmp_dst = []
...
def groupStreams(streams):
	...
	for stream in streams:
		...
		if not tmp_set:
			...
			if (int(stream) - int(tmp_set[-1])) < 10 and length != len(streams):
				tmp_set.append(stream)
...
def findAssociatedPath(associatedStream, stream_list, time_list):
	...
	try:
		lte_failed = open(tmp[0] + "_LTE_failed_report.csv", 'r')
		lte_passed = open(tmp[0] + "_LTE_passed_report.csv", 'r')
		other_failed = open(tmp[0] + "_Other_failed_report.csv", 'r')
		other_passed = open(tmp[0] + "_Other_passed_report.csv", 'r')

		lte_failed_reader = csv.reader(lte_failed)
		lte_passed_reader = csv.reader(lte_passed)
		other_failed_reader = csv.reader(other_failed)
		other_passed_reader = csv.reader(other_passed)
		...
		if isNotFound:
			for row in lte_failed_reader:
				...
				if (abs(unix_time - tmp_time) < 5) and checkDriverKitID(row[3]):
					...
					print("Time from the failed/passed report = ", tmp_time)
					path = row[PATH][:-4]
					isNotFound = False
					break

		if isNotFound:			
			for row in lte_passed_reader:
				tmp_time = dateToUnixTime(row[UTC_TIME])
		...
		if isNotFound:
			for row in other_failed_reader:
				tmp_time = dateToUnixTime(row[UTC_TIME])
				same_flag = checkDriverKitID(row[3])
				if (abs(unix_time - tmp_time) < 5) and checkDriverKitID(row[3]):
					print("Time from the stream.csv = ", unix_time)
		...
		if isNotFound:
			for row in other_passed_reader:
				tmp_time = dateToUnixTime(row[UTC_TIME])
				same_flag = checkDriverKitID(row[3])
				if (abs(unix_time - tmp_time) < 5) and checkDriverKitID(row[3]):
					print("Time from the stream.csv = ", unix_time)
